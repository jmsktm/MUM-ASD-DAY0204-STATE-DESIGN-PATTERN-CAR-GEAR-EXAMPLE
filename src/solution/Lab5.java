...
public class Lab5 extends javax.swing.JFrame {
    ...
    void JSlider1_stateChanged(javax.swing.event.ChangeEvent event) {
        ...
        if (!source.getValueIsAdjusting()) {
            ...
            Shift shift = stateContext.checkShift(mph);
            JTextFieldSpeed.setText(String.valueOf(mph));

            int newShift = shift.value();
            if (newShift > prevShift) {
                JSlider1.setValue(shift.min());
            } else if (newShift < prevShift) {
                JSlider1.setValue(shift.max());
            }
            prevShift = newShift;

            if (newShift > 0) {
                JTextFieldShift.setText("" + newShift);
            } else {
